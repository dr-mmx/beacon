...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("beacon.beacon.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  Whois(request: QueryGetWhoisRequest): Promise<QueryGetWhoisResponse> {
    const data = QueryGetWhoisRequest.encode(request).finish();
    const promise = this.rpc.request("beacon.beacon.Query", "Whois", data);
  ...
  WhoisAll(request: QueryAllWhoisRequest): Promise<QueryAllWhoisResponse> {
    ...
    const promise = this.rpc.request("beacon.beacon.Query", "WhoisAll", data);
